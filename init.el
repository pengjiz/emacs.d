...
(confige erc
  :preload t
  (:preface
   ;; NOTE: For some unknown reason the compiler complains that this function
   ;; may not be defined at runtime, but it is actually there.
  ...
  (:postface
   ...
   (confige erc-services
     :preload t
     (:before (setf erc-nickserv-identify-mode 'autodetect))
     (:after (setf erc-use-auth-source-for-nickserv-password t)))
   ...
   (confige erc-truncate
     :preload t
     (:after (setf erc-max-buffer-size 100000)))))

(confige erc-extras
  (:preface (autoload 'erc-extras-setup "erc-extras"))
  (:before
